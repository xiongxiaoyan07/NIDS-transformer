...
class TimeAwareEncoding(nn.Module):
    ...
    def __init__(
        ...
        # self.lambda_pos = nn.Parameter(torch.tensor(1.0))
        # self.lambda_time = nn.Parameter(torch.tensor(1.0))
    ...
    def _time_aware_encoding(self, seq_len: int, p: torch.Tensor) -> torch.Tensor:
        ...
        # combined = (
        #         self.lambda_pos * pos.unsqueeze(0)
        #         + self.lambda_time * p.unsqueeze(-1)
        # )
...
class AttentionPooling(nn.Module):
    ...
    def forward(
        ...
        mask_f = (mask_bool.unsqueeze(-1).to(dtype=h.dtype))
        ...
        attention_pool = (h * weights).sum(dim=1)

        # Mean pooling
        count = mask_f.sum(dim=1).clamp(min=1.0)

        mean_pool = (h * mask_f).sum(dim=1) / count

        # Max pooling
        max_input = h.masked_fill(~mask_bool.unsqueeze(-1), torch.finfo(h.dtype).min)
...
#         )
#
#         # 使用 scalar gate，先减少参数和过拟合风险
#         self.gate_layer = nn.Linear(2 * d_model, 1)
#
#         # 初始 sigmoid(-3) ≈ 0.047：
#         # 模型开始时基本等价于 packet-only
#         nn.init.zeros_(self.gate_layer.weight)
#         nn.init.constant_(self.gate_layer.bias, -3.0)
#
...
#         )  # [B, 1]
#
#         flow_delta = self.flow_delta(flow_encoded)
#
#         fused = pooled + gate * flow_delta
#
...
class ResidualGatedFlowFusion(nn.Module):
    def __init__(
        ...
        # 让 residual 初始严格等于 0
        final_linear = self.flow_delta[-1]
        ...
        nn.init.zeros_(self.gate_layer.weight)
        nn.init.constant_(self.gate_layer.bias, -2.5)
    ...
    def forward(
        ...
        # 整个 flow modality 随机关闭，而不是普通逐维 dropout
        if self.training and self.modality_dropout > 0:
            ...
            ).to(flow_encoded.dtype)

            flow_encoded = flow_encoded * keep_mask
        ...
        # 不再额外做 LayerNorm
        # classifier 已经包含 LayerNorm
        return pooled + gate * flow_delta
# ============================================================
...
class Stage1TimeAwareTransformer(nn.Module):
        ...
        if self.use_flow_fusion and not self.inject_to_packets:
            ...
            if flow_feature_dim > 0:
                ...
                if self.fusion_method == "residual_gated":
                    self.residual_fusion = ResidualGatedFlowFusion(
                        d_model=d_model,
                        dropout=dropout,
                    )
                else:
    ...
    def forward(
        ...
        if self.use_flow_fusion and not self.inject_to_packets and flow_feats is not None:
            ...
            if self.fusion_method == "residual_gated":
                z = self.residual_fusion(z,flow_encoded)
            else:
