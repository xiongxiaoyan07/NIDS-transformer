...
class Stage1TimeAwareTransformer(nn.Module):
    ...
    def __init__(self, input_dim: int, cfg: Dict[str, Any]):
        ...
        if self.use_flow_fusion and not self.inject_to_packets:
            ...
            if flow_feature_dim > 0:
                # === Flow Token Projection ===
                print("[*************Stage1TimeAwareTransformer --- init----flow_feature_dim= ]",flow_feature_dim)
                self.flow_token_proj = nn.Sequential(
                    nn.LayerNorm(flow_feature_dim),
                    nn.Linear(flow_feature_dim, d_model),
                    nn.GELU(),
    ...
    def forward(
        ...
        # 3. ===== FLOW TOKEN VERSION =====
        if flow_feats is not None:
            flow_token = self.flow_token_proj(flow_feats).unsqueeze(1)  # [B, 1, D]
            # scale down (关键)
            flow_token = 0.1 * flow_token
            # prepend flow token
            e = torch.cat([flow_token, e], dim=1)  # [B, L+1, D]

            # mask also extend (flow token is always valid)
            flow_mask = torch.ones(
                (mask.size(0), 1),
                device=mask.device,
                dtype=mask.dtype,
            )
            mask = torch.cat([flow_mask, mask], dim=1)
