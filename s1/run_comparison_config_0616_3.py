...
def run_single_experiment(args, config: dict):
    ...
    # 5. 16头注意力
    config_batchsize32 = copy.deepcopy(config)
    config_batchsize32['training']['batch_size'] = 32
    benchmark.models['Ours_Batchsize32'] = Stage1TimeAwareTransformer(
        input_dim=input_dim,
        cfg=config_batchsize32
    )
    print("[✓] Ours - Transformer batchsize 32")

    config_batchsize64 = copy.deepcopy(config)
    config_batchsize64['training']['batch_size'] = 64
    benchmark.models['Ours_Batchsize64'] = Stage1TimeAwareTransformer(
        input_dim=input_dim,
        cfg=config_batchsize64
    )
    print("[✓] Ours - Transformer batchsize 64")

    config_batchsize128 = copy.deepcopy(config)
    config_batchsize128['training']['batch_size'] = 128
    benchmark.models['Ours_Batchsize128'] = Stage1TimeAwareTransformer(
        input_dim=input_dim,
        cfg=config_batchsize128
    )
    print("[✓] Ours - Transformer batchsize 128")

    config_batchsize256 = copy.deepcopy(config)
    config_batchsize256['training']['batch_size'] = 256
    benchmark.models['Ours_Batchsize256'] = Stage1TimeAwareTransformer(
        input_dim=input_dim,
        cfg=config_batchsize256
    )
    print("[✓] Ours - Transformer batchsize 256")

    config_batchsize512 = copy.deepcopy(config)
    config_batchsize512['training']['batch_size'] = 512
    benchmark.models['Ours_Batchsize512'] = Stage1TimeAwareTransformer(
        input_dim=input_dim,
        cfg=config_batchsize512
    )
    print("[✓] Ours - Transformer batchsize 512")
