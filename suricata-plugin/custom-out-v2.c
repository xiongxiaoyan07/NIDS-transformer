 ...
 * New fields:
 *   flow_iat_us      = time gap from previous packet in the same bidirectional flow.
 *   direction_iat_us = time gap from previous packet in the same direction.
 */
 ...
 * Bulk and Subflow features are intentionally removed because they require more CICFlowMeter-specific logic.
 * flow_id and source/destination tuple are kept for debugging and joining with packet CSV.
 * Drop these columns during model training if you want CIC-IDS2017-like ML inputs.
 */
