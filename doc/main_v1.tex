...
\tableofcontents
\begin{abstract}
Flow-level network intrusion detection commonly represents each connection by
aggregate statistics and classifies it independently. Although efficient, this
formulation can discard packet order and irregular timing and can miss evidence
distributed across related flows. This thesis proposes a hierarchical,
payload-content-independent detector that preserves both structures. A custom
Suricata output module extracts aligned packet records and flow statistics from
an enterprise PCAP. Stage~1 encodes ordered packet metadata with positional and
cumulative-time information, conditions the packet tokens on flow statistics,
and exports one fused representation per flow. Stage~2 uses the target flow to
query a causal history of earlier related flow embeddings before making the
final prediction.

The evaluation uses chronological partitions and emphasizes macro-F1, PR-AUC,
attack-class performance, confusion matrices, and false-positive rate because
the positive class is rare. On the controlled 35,410-flow Stage~1 test
manifest, the complete fused representation obtains macro-F1 0.8955 and PR-AUC
0.8763, outperforming aggregate-flow, recurrent, convolutional, and encoding
ablations. On the separate 84,380-flow Stage~2 manifest, causal source-host
context increases macro-F1 from 0.8954 to 0.9275 and PR-AUC from 0.8763 to
0.9345, while reducing both false positives and false negatives. The proposed
target-query model also outperforms no-context, vanilla Transformer, LSTM, GRU,
and CNN--LSTM controls with model-level latency comparable to the vanilla
Transformer. These results support explicit intra-flow timing and causal
inter-flow context as complementary sources of evidence. The conclusions are
limited to one alert-labelled organizational capture and predominantly
single-seed experiments.

  \begin{keywords}
   Network intrusion detection, packet sequence, time-aware Transformer,
   flow representation, cross-flow context, class imbalance
  \end{keywords}
\end{abstract}
...
This thesis addresses the gap with a hierarchical two-stage framework. A custom
Suricata output module converts raw PCAP traffic into aligned packet records and
flow statistics. Stage~1 learns one flow representation from packet order,
elapsed time, and aggregate statistics. Stage~2 then refines the prediction
using a bounded causal history of related flow representations. This separation
makes it possible to test intra-flow representation learning and inter-flow
contextual reasoning independently while retaining a single flow-level output.
...
\begin{itemize}
    \item An aligned PCAP-to-learning pipeline that extracts packet-level and
    flow-level records through a custom Suricata output module while separating
    model predictors, context identifiers, and alert-derived targets.
    \item A Stage~1 flow representation that combines packet position,
    cumulative elapsed time, mask-aware self-attention, and bounded
    flow-conditioned token modulation.
    \item A causal Stage~2 target-query attention model that retrieves evidence
    from relation-specific histories without exposing future flows or
    historical labels.
    \item A controlled evaluation of representation, fusion, sequence length,
    context relation, context capacity, architecture, and computational cost
    under class imbalance.
\end{itemize}

Because the positive class is rare, the evaluation prioritizes macro-F1,
PR-AUC, attack-class performance, and false-positive rate over accuracy alone.

The remainder of the thesis is organized as follows.
Chapter~\ref{sec:related_work} reviews packet-, flow-, Transformer-, and
context-based intrusion detection. Chapter~\ref{sec:dataset_preprocessing}
describes data construction and leakage-aware preprocessing.
Chapter~\ref{sec:proposed_methodology} defines the proposed two-stage model,
and Chapter~\ref{sec:experimental_methodology} specifies the controlled
evaluation protocol. Chapter~\ref{sec:results} reports and discusses the
results. Chapter~\ref{sec:conclusion_future_work} concludes the thesis and
...
by microseconds or seconds. Second, packet sequences and aggregate flow
statistics describe complementary aspects of a connection. Third, a
representation learned for one flow does not by itself indicate whether that
flow forms part of a distributed host-level pattern. These distinctions define
the representational gap synthesized at the end of this chapter.
...
\cite{Huang2024CoTNet}, and DoLLM represents flow records in a form suitable
for large-language-model-based DDoS detection \cite{Li2024DoLLM}. These
approaches show that useful representations can be learned without plaintext
payloads. However, encrypted-traffic classification and intrusion detection are
not identical: application identity may remain stable even when malicious
behavior is sparse, evolving, or distributed across connections.
...
payload-independent intra-flow packet modelling, explicit elapsed-time
encoding, flow-statistical fusion, and configurable inter-flow context. This
comparison therefore motivates the hierarchical separation between Stage~1 and
Stage~2.
...
This thesis addresses that gap with a hierarchical division of labor. Stage~1
models packet order and elapsed time within each flow and late-fuses the learned
sequence representation with flow statistics. Stage~2 then models dependencies
...
The next chapter establishes the aligned packet--flow dataset and leakage
controls required to evaluate that hierarchy.
...
This chapter describes how the proprietary packet capture was converted into
...
\datasetfield{label} is included in a feature vector, and Suricata alert
metadata is absent from the model input. Exact reproduction of the weak-labeling
function would additionally require the Suricata version, ruleset snapshot,
custom-module revision, configuration, and PCAP checksum. The unavailable parts
of this provenance record are disclosed in
Chapter~\ref{sec:experimental_methodology} rather than reconstructed after the
experiments.
...
interpretation. The study evaluates a traceable model pipeline on a large
...
The resulting tensors, masks, flow statistics, and causal context metadata form
the data contract for the two model stages defined in the next chapter.
...
This chapter presents the proposed hierarchical intrusion-detection method in
...
Chapter~\ref{sec:dataset_preprocessing}.
...
Here, ``causal'' is defined by the implementation's stable start-time order:
every context index precedes the target index. The current Stage~2 export does
not enforce $t_j^{\mathrm{end}}\leq t_i^{0}$, so long-lived overlapping flows
may appear earlier in that order even if their complete embeddings would not
yet be available at the target start time. The experiments therefore establish
start-ordered, label-free context rather than a fully timed streaming
implementation. A deployment-grade version should construct eligibility from
flow-completion or embedding-availability timestamps.
...
a $W\times W$ score matrix. This makes a window of 128 flows practical while
...
\cite{Lin2017FocalLoss}. The focal exponent, class weights, and label-smoothing
settings are experimental choices and are reported in
Table~\ref{tab:experimental_training_config}.
...
The sampler changes expected minority exposure without altering the dataset or
the loss definition. Its stage-specific targets and loader policy belong to the
experimental protocol in Chapter~\ref{sec:experimental_methodology}.

\subsection{Sequential Stage Interface}
\label{subsec:sequential_stage_interface}

The stages are optimized sequentially. Stage~1 is first trained for flow-level
classification and then restored from its validation-selected checkpoint. A
deterministic, non-sampled loader exports exactly one
$\mathbf{z}^{(1)}_i$ per flow together with the metadata required for causal
context construction. Stage~2 treats these embeddings as fixed inputs; its
gradients do not update the Stage~1 encoder.

This separation makes the source of an improvement auditable: Stage~1
experiments change the intra-flow representation, whereas Stage~2 experiments
change contextual reasoning over one frozen representation. The exact
optimizer, sampler, checkpoint, and decision-threshold settings are therefore
reported once in Chapter~\ref{sec:experimental_methodology} rather than being
repeated here.
...
principal experiments. Named ablations change only the factor identified in the
experimental matrix; all optimization settings are reported separately in
Chapter~\ref{sec:experimental_methodology}.

\begin{table}[H]
    \centering
    \caption{Principal architecture configuration.}
...
Stage~1 combines cumulative-time encoding, bounded flow-conditioned token
modulation, and mask-aware hybrid pooling to produce one flow representation.
Stage~2 applies a gated target-query update over relation-controlled historical
embeddings, with an exact fallback when no history exists. The next chapter
evaluates these mechanisms through controlled baselines and ablations under one
leakage-aware training and threshold-selection protocol.

\providecommand{\ExpPlatform}{Google Colaboratory Pro+}
\providecommand{\ExpComputeBudget}{600 purchased compute units}
\providecommand{\ExpGPU}{NVIDIA T4 (16~GB) for Stage~1; NVIDIA L4 (24~GB) for Stage~2}
...
Dataset construction and model equations are defined in
Chapters~\ref{sec:dataset_preprocessing} and
\ref{sec:proposed_methodology}, respectively. This chapter specifies only the
comparisons, fixed controls, optimization and threshold rules, evaluation
metrics, and computational measurements used to answer the research questions
introduced in Chapter~\ref{sec:introduction}. Stage~1 experiments test the
value of packet order, elapsed time, and flow-statistical fusion; Stage~2
experiments test whether causal histories improve prediction over independent
flow classification.
\section{Dataset Partition and Leakage Controls}
\label{subsec:experimental_data_protocol}

The archived experiments contain two chronological evaluation blocks. Stage~1
architecture, encoding, fusion, sequence-length, and selection comparisons use
a 35,410-flow test manifest containing 34,003 negative and 1,407 positive
flows. The independent Scheme~C reference used in the hierarchical comparison
and all Stage~2 experiments use the full 84,380-flow test partition containing
80,860 negative and 3,520 positive flows. The two
blocks are not treated as paired evaluations, even when rounded metrics are
similar. Effect sizes are reported only within one manifest.

Within each block, models receive the same flow universe, packet and flow
tensors, split membership, and training-fitted preprocessing state. Categorical
vocabularies, clipping bounds, and scaling parameters are fitted on training
data only. Sampler weights use training counts, while configured class weights
and sampling targets are selected without test feedback; validation and test
data never fit transformations.

Stage~2 uses one frozen Stage~1 checkpoint. Its online context policy permits
only embeddings earlier in the stable start-time order: validation examples may
retrieve earlier training flows, and test examples may retrieve earlier
training, validation, and test flows. Histories are updated only after the
current context has been built, and neither future embeddings nor historical
labels are retrieved. Every Stage~2 architecture comparison reuses the same
embedding files, metadata order, context indices, and masks. These invariants
isolate the varied model or context factor from preprocessing and context
availability. Section~\ref{subsubsec:context_relations} states the distinction
between this start-ordered policy and strict flow-completion-time availability.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lllll@{}}
        ...
        \midrule
        S1-E1 & 1 & 35,410-flow Stage~1 manifest & Flow MLP; packet
        ...
        Truncation policy \\
        S2-E1 & 2 & 84,380-flow manifest; same frozen embeddings & No-context MLP;
...
\begin{itemize}[leftmargin=*]
    ...
    embedding and provides a minimal current-flow-only lower bound. Because it
    is not capacity matched to the contextual encoders, it is not by itself a
    causal estimate of the value of history.
...
$W\in\{16,32,64,128,256\}$. The available run summaries retain predictive
metrics and confusion matrices but not per-target context occupancy or context
age. The resulting limitation is addressed explicitly in the interpretation of
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}p{3.0cm}p{4.6cm}p{5.7cm}@{}}
        ...
        Loss & Focal, $\gamma=1$, $\boldsymbol{\alpha}=[1,1.25]$, no label
        smoothing & Focal, $\gamma=1$, $\boldsymbol{\alpha}=[1,1.01]$, no label
        smoothing \\
...
\subsubsection{Validation-Only Decision Threshold}
\label{subsubsec:experimental_thresholding}

Checkpoint selection and decision-threshold selection are separate. The
principal Scheme~C and Stage~2 runs select the checkpoint with the highest
validation PR-AUC. After restoring that checkpoint, the class-1 decision
...
\end{equation}
For Stage~1, $\Delta$ is the set of thresholds induced by the validation
precision--recall curve. Stage~2 evaluates 199 equally spaced values from 0.01
to 0.99. The selected $\delta^{*}$ is frozen and applied once to the test
probabilities. Test-optimized thresholds are retained only as diagnostics and
are excluded from the reported deployable results. The Stage~1 architecture
benchmark uses its shared comparison trainer, which checkpoints all rows by
validation class-1 F1; those rows are therefore compared only within that
benchmark, as specified in Section~\ref{subsubsec:stage1_baselines}.
...
Experiments were executed in Google Colaboratory Pro+ using a company-funded
allocation of 600 compute units. Compute units describe access to hosted
resources rather than a fixed amount of GPU work \cite{GoogleColabFAQ2026}.
Stage~1 model-family runs reported in this thesis used an NVIDIA T4 (16~GB),
whereas the Stage~2 architecture profiles used an NVIDIA L4 (24~GB)
\cite{NVIDIAT4Datasheet,NVIDIAL4ProductBrief}. Runtime is compared only within
one stage and accelerator block; T4 and L4 wall-clock values are never ranked
against each other.

Trainable parameter count is obtained directly from model tensors requiring
gradients. Stage~1 training time covers the complete fit loop, including
validation and checkpointing, and is reported as a descriptive single-run cost
because model-specific early stopping and hosted-runtime variation affect total
wall time. Stage~2 inference is measured with gradients disabled and the model
in evaluation mode. The reported per-flow latency is
\begin{equation}
...
\end{equation}
where $Q$ is throughput in flows per second. This profile begins after Stage~1
embeddings and context indices have been constructed; it is model-evaluation
latency, not end-to-end PCAP-to-alert latency.
...
The exact CPU, software-version snapshot, and Suricata revision were not
retained in the available run metadata and are disclosed as such rather than
reconstructed retrospectively. This limits exact runtime replication but does
not alter comparisons performed within the recorded T4 and L4 blocks. With the
comparison factors, threshold policy, metrics, and timing scope now fixed, the
next chapter reports the resulting evidence in the order of the experimental
matrix.
...
This chapter reports and interprets the experiments defined in
Chapter~\ref{sec:experimental_methodology}. Exact model equations, data
transformations, and metric definitions are not repeated. Unless stated
otherwise, F1, precision, and recall denote macro averages; class-1 measures
are identified explicitly. Interpretation emphasizes macro-F1, PR-AUC,
attack-class performance, FP, FN, and FPR because accuracy is dominated by the
negative class.

Two test manifests are used and are kept analytically separate. Stage~1 model,
encoding, fusion, sequence-length, and packet-selection comparisons use 35,410
flows (34,003 negative and 1,407 positive). The complete hierarchical comparison
uses a separate 84,380-flow manifest (80,860 negative and 3,520 positive) whose
Stage~1 embeddings are shared by all Stage~2 models. Effect sizes are calculated
only between rows evaluated on the same manifest and protocol.

The archived summaries retain PR-AUC (average precision) for every run, but do
not retain aligned per-flow probabilities for every model. PR-AUC is therefore
shown as a scalar bar or sensitivity line. A genuine precision--recall curve is
not reconstructed from aggregate values because its shape depends on predictions
...
encoding ablations on the common flow test set. A figure is used instead of a
wide predictive-performance table because the main question is the relative
pattern across macro-F1, class-1 F1, and PR-AUC. Exact computational
measurements are retained separately in
...
The position+time Transformer provides the strongest balance in this block,
reaching macro-F1 $0.8882$ and PR-AUC $0.8564$. Relative to LSTM+time, the
strongest non-Transformer baseline by macro-F1, it gains 2.05 percentage points
of macro-F1 and recovers 95 additional positive flows for 10 additional false
positives. The gain is therefore operationally meaningful rather than a change
in a ranking metric alone.

The ablation clarifies why both coordinates are needed. Position identifies
where a packet occurs in the selected sequence, whereas cumulative time
distinguishes a rapid burst from the same order spread over a longer interval.
Time-only is stronger than position-only, but their combination adds a further
0.64 percentage points of macro-F1 and improves both error types over no
encoding. This pattern is consistent with the hypothesis that ordinal and
physical time describe complementary traffic behavior; the single-capture
experiment does not establish that every attack family uses them equally.

The GRU obtains the lowest FPR ($0.8293\%$), but misses 437 positive flows.
The position+time model adds only 14 FP relative to GRU while removing 131 FN.
This comparison shows why a low false-alarm rate cannot be interpreted without
positive coverage: a conservative detector can suppress alerts by moving errors
from FP to FN.

The flow-statistics MLP is the direct test of whether packet dynamics add
information beyond aggregation. On the same test composition, its macro-F1 and
PR-AUC are $0.7762$ and $0.5604$, versus $0.8882$ and $0.8564$ for the
position+time model. FP falls from 853 to 296 and FN from 501 to 306. Aggregate
duration, count, and inter-arrival summaries therefore do not recover all of the
ordering and local timing evidence preserved by the packet sequence.

Figure~\ref{fig:results_stage1_errors} visualizes these operating-point
trade-offs; because every model uses the same 34,003 negative examples, its
false-positive bars are directly proportional to FPR.
...
Explicit time also improves LSTM, removing 29 FP and 22 FN. The CNN result is
mixed: adding time improves its selected operating point but lowers PR-AUC and
adds FN. This divergence matters because thresholded F1 describes one deployed
operating point, whereas PR-AUC describes score ordering across thresholds.
The benefit of a timing feature therefore depends on how the encoder uses it;
providing the feature alone is not sufficient.
...
Scheme B outperforms Scheme A by 1.30 percentage points of macro-F1 and 2.38
points of PR-AUC. Repeating one aggregate vector at every packet position can
therefore interfere with token-specific evidence even though it slightly raises
macro-precision. Aggregate statistics are useful, but not when they are treated
as if they were new observations at every sequence position.

Scheme C provides the strongest result, with macro-F1 $0.8955$, PR-AUC
$0.8763$, 267 FP, and 293 FN. Its improvement over packet-only Scheme B occurs
in both error directions, supporting the use of a separate statistical pathway
that conditions rather than replaces packet tokens. This experiment validates
the complete Scheme C design. It does not fully isolate token-FiLM from its
additional parameters and late-fusion path; a parameter-matched concatenation
control would be required to attribute the gain to the modulation mechanism
alone.
...
Performance is non-monotonic in sequence length. Very short inputs provide
weaker ranking performance, indicating that the first few packets do not always
contain enough evidence. Extending the budget to 128 gives the best macro-F1
and error balance, whereas 256 adds 46 FP and lowers both macro-F1 and PR-AUC.
More packets can therefore add stale, repetitive, or weakly relevant evidence
rather than useful context.

This interpretation remains a hypothesis because the sweep changes the amount
of truncation differently for short and long flows. A length-stratified analysis
would be needed to distinguish an optimization effect from genuine dilution by
later packets. The selected $L=128$ should therefore be described as the best
observed budget for this capture, not a universal packet horizon.
Head and head-tail selection are practically indistinguishable in a single-seed
comparison. Random selection lowers FPR but increases FN, again favoring a more
conservative operating point. Head selection is retained because it provides
the strongest observed macro-F1 and positive coverage, is deterministic, and
uses only the earliest packets available to a causal detector. No strong claim
is made from the 0.16-point difference between head and head-tail.
...
The complete Stage~2 system raises macro-F1 by 3.21 percentage points and
PR-AUC by 5.82 points over independent Scheme C classification. It removes 228
FP and 205 FN, reducing FPR by approximately 34.2\% relative. Improvement in
both error types is stronger evidence than accuracy alone that historical flows
add useful decision information. This comparison evaluates the hierarchical
system as a whole; the architecture controls below are needed to separate
contextual evidence from additional classifier capacity.
...
Source-host context gives the strongest balance, with macro-F1 $0.9275$,
PR-AUC $0.9345$, and the lowest FPR. A plausible explanation is that repeated
outbound behavior from one initiator preserves scanning, retry, or coordinated
connection patterns, whereas global recency mixes unrelated traffic and a
destination history can mix many benign clients of a popular service. The
result is consistent with relational and window-level NIDS findings
\cite{Deng2023FlowTopology,MartinReizabal2026FlowWindow}, but the current binary
labels cannot verify which attack family produces the advantage.

Endpoint context occupies a high-recall regime: it misses fewer positives but
produces 702 FP. By accepting either endpoint in either role, it increases the
chance of retrieving relevant lateral or bidirectional behavior, but also
imports a more heterogeneous neighborhood. It may therefore be preferable when
missed attacks dominate alert-review cost, whereas source-host is the better
default under the thesis's balanced criterion. Time-only context performs worse
in ranking and FN, indicating that proximity without an entity relation is too
weak a context definition for this capture. These trade-offs motivate the
relation-aware extension proposed in Chapter~\ref{sec:conclusion_future_work}.
...
length is non-monotonic. A 32-flow window produces the highest macro-recall,
but its 775 false positives indicate that the added sensitivity is operationally
costly. The 128-flow window instead gives the best joint result in macro-F1
($0.9275$), PR-AUC ($0.9345$), and FPR ($0.5429\%$). Extending the window to
256 degrades both false-positive and false-negative counts. This pattern is
consistent with a finite useful context horizon: too little history can omit a
behavioural pattern, whereas older same-source flows may be unrelated to the
current event and dilute the attention signal.

This interpretation remains a hypothesis rather than a direct measurement of
attention quality. The current aggregate results do not retain the number and
age distribution of valid context items per target flow. Consequently, they
cannot distinguish whether the changes arise from genuinely older evidence,
different padding rates, or unequal history coverage. A follow-up analysis
should stratify performance by observed context length and elapsed context span.
Within the tested design, $W=128$ is therefore the best empirical operating
point, not a universal optimum.
...
The target-query model provides the strongest overall balance: it reaches
macro-F1 $0.9275$, class-1 F1 $0.8610$, and PR-AUC $0.9345$. The closest
baseline values are $0.9097$ macro-F1 from CNN+LSTM and $0.9141$ PR-AUC from
GRU. LSTM attains marginally higher macro-recall, but this isolated advantage
does not translate into the best threshold-independent ranking or the best
false-alert balance.

The confusion matrices clarify the practical meaning of this ranking. Relative
to the vanilla Transformer, the proposed model removes 177 false positives and
117 false negatives on the same test manifest. LSTM detects 28 additional
positive flows, but produces 338 additional false alerts. Thus, the proposed
model's macro-F1 improvement is not obtained by simply shifting the threshold
towards conservative positive prediction; it reduces both error types against
the vanilla attention baseline and substantially improves alert precision over
the highest-recall recurrent baseline.

The comparison with vanilla self-attention is consistent with the intended
inductive bias. Vanilla attention repeatedly updates the context sequence and
classifies its final valid state. The proposed model instead asks a narrower
question: which earlier related flows are relevant to the current flow? It uses
the target as the query and gates the resulting context update. The simultaneous
gain in macro-F1 and PR-AUC suggests that this target-centred computation ranks
minority examples more effectively, although the comparison cannot isolate the
query pattern from all architectural and parameter-count differences.

The no-context head is useful as a current-flow-only lower bound, but not as a
capacity-matched ablation: it contains 514 parameters, compared with 283,522 in
the proposed Stage~2 model. Its weaker result shows that a minimal independent
classifier is insufficient, but the complete performance difference cannot be
attributed exclusively to historical context. A stronger causal test would
retain the target-query network and parameter count while masking every history
item. This distinction prevents the architectural comparison from overstating
...
All Stage~2 profiles in Figure~\ref{fig:results_stage2_efficiency} were collected
on an NVIDIA L4 accelerator. The proposed model contains 283,522 parameters and
requires 0.1965~ms per flow for the measured model-evaluation path, corresponding
to approximately 5,089 flows per second. Its latency is effectively identical
to the vanilla Transformer's 0.1967~ms; the 0.0002~ms difference is below the
resolution needed for a meaningful speed claim. The relevant result is therefore
performance parity in latency while obtaining a stronger error balance, not an
acceleration over vanilla attention.
...
lower by $42.2\%$, $43.3\%$, and $45.2\%$. Unlike the negligible difference
from vanilla attention, these margins indicate a meaningful compactness and
model-level latency advantage over the recurrent alternatives. They should not,
however, be interpreted as end-to-end deployment throughput. The measurement
excludes context-index construction, Stage~1 inference, Suricata extraction,
and PCAP processing, and a single aggregate timing does not quantify warm-up
effects, run-to-run variance, batching sensitivity, or tail latency.

\section{Cross-Stage Discussion}
\label{subsec:results_cross_stage_discussion}

The two stages address different information losses. Stage~1 recovers packet
order and elapsed time that disappear when a flow is reduced to aggregate
statistics; Stage~2 recovers behavioural evidence that is unavailable when
flows are classified independently. The ablations support both parts of this
decomposition. Position and time are complementary within a flow, while causal
source-host history improves the complete detector after the Stage~1
representation has been fixed. This hierarchical result extends the motivation
of packet-aware systems such as GTID and modular flow encoders such as
FlowTransformer by testing an additional, explicit cross-flow reasoning layer
\cite{Han2023GTID,Manocchio2024FlowTransformer}.

For RQ1, the evidence supports time-aware packet-sequence modelling over the
flow-statistics MLP on the shared Stage~1 manifest, and the fusion study suggests
that aggregate statistics remain useful when they condition rather than replace
the packet representation. The non-monotonic sequence-length response is equally
important: the gain follows from selecting and encoding informative packets, not
from increasing the token budget without limit. For RQ2, the aligned 84,380-flow
comparison shows that Stage~2 lowers both false-positive and false-negative
counts relative to independent Scheme~C predictions. The relation and window
ablations further indicate that context must be selective; merely exposing a
model to more neighbouring flows is insufficient.

The combined findings are consistent with broader temporal and relational NIDS
work, including sequential traffic modelling and graph-based flow analysis
\cite{LiuPatras2022NetSentry,Deng2023FlowTopology,Xu2024SSLGNN}. The present
experiments do not show that one fixed source-host relation captures every
attack mechanism, nor do binary labels reveal which attack families benefit.
An aligned error-transition analysis would strengthen the cross-stage argument
by identifying which Stage~1 false positives and false negatives are corrected
by Stage~2 and by relating those corrections to context size, age, and relation.

\section{Reliability and Validity}
\label{subsec:results_reliability}

The experiments are predominantly single-seed runs using seed 130. They support
controlled ablation and sensitivity observations, but they do not estimate
training variability. Neural model rankings can change with initialization and
mini-batch order \cite{ReimersGurevych2017,Bouthillier2021Variance}; small gaps,
including the head versus head-tail difference, should therefore be treated as
descriptive. The larger Stage~2 gains are stronger empirical evidence, but no
claim of statistical significance is made.

Confidence intervals and paired tests cannot be reconstructed from aggregate
metrics. They require aligned per-flow probabilities and predictions for every
run. Moreover, all results derive from one organizational capture with binary,
alert-derived labels, so they establish in-domain effectiveness rather than
cross-network or cross-time generalization. Repeated seeds, retained prediction
files, temporally later captures, and external datasets are consequently the
main requirements for confirmatory evaluation.

\chapter{Conclusion and Future Work}
\label{sec:conclusion_future_work}

This thesis investigated whether network intrusion detection benefits from
preserving packet timing within flows and chronological relationships among
flows. It developed a hierarchical, payload-independent detector in which
Stage~1 combines ordered packet metadata, elapsed time, and flow statistics,
and Stage~2 retrieves earlier relation-specific flow embeddings. The controlled
experiments support these two structures as complementary sources of
minority-class evidence, within the capture, label, and repeatability boundaries
stated in Chapters~\ref{sec:dataset_preprocessing},
...
The controlled Stage~1 evidence supports modelling packets as an ordered,
time-aware sequence rather than relying only on flow aggregates. The combined
position-and-time encoder outperforms the flow-statistics MLP by $0.1120$
macro-F1 and $0.2960$ PR-AUC on the shared manifest, while the encoding
ablations show that ordinal position and elapsed time contribute complementary
information. Flow-conditioned token modulation improves the final Stage~1
representation further. The sequence-length results qualify this conclusion:
the best tested setting is a 128-packet head sequence, and additional tokens do
not produce monotonic gains.

\paragraph{RQ2: cross-flow context.}
The aligned Stage~2 comparison supports causal contextual reasoning beyond
independent flow classification. Source-host context raises macro-F1 from
$0.8954$ to $0.9275$ and PR-AUC from $0.8763$ to $0.9345$, while reducing both
false positives and false negatives. The relation and window ablations show
that this gain depends on context selection: a 128-flow source-host history
provides the best balance in the tested environment, whereas endpoint context
favours recall at a higher false-alert cost. The target-query model also
outperforms the tested recurrent, convolutional--recurrent, vanilla-attention,
and minimal no-context alternatives, although the no-context comparison is not
capacity matched.

Taken together, the results answer the central research question by showing
that the two structural levels are complementary. Packet timing and order make
the individual-flow representation more informative, and selected earlier
flows resolve ambiguity that remains after Stage~1. The thesis contribution is
therefore the evaluated hierarchy and its controlled decomposition, rather than
the use of a Transformer in isolation.

\section{Limitations and Scope}
\label{subsec:conclusion_limitations}

The conclusions are bounded by one organizational capture, alert-derived binary
labels, and predominantly single-seed neural experiments. They establish
in-domain effectiveness but not transfer across networks, time periods, attack
families, labeling policies, or unseen encrypted protocols. Small differences
between nearby configurations remain descriptive until repeated runs and paired
uncertainty estimates are available. Sequential training with frozen Stage~1
embeddings also favours experimental separation over end-to-end optimization.
Finally, the reported Stage~2 latency covers model evaluation after embeddings
and context indices are available; it does not measure the complete
PCAP-to-alert path, production memory, tail latency, or concept drift.
...
raw IP addresses and ports should be removed or carefully randomised during
...
a target-conditioned gate. In either design, only embeddings available at the
target's decision time should remain eligible. Enforcing this with flow-end or
embedding-availability timestamps would strengthen the current start-ordered
constraint. The context horizon could also be adaptive: a model could choose
...
Overall, the study shows that packet timing and related-flow context contain
evidence that is lost when traffic is reduced to independent aggregate rows.
The next step is to verify this structural advantage across seeds, captures,
and operational conditions, then strengthen it through resource-efficient
pre-training and attack-adaptive relation-aware context.
