...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lllll@{}}
        ...
        \midrule
        S1-E1 & 1 & 35,410-flow Stage~1 manifest & Flow MLP; packet sequence models; proposed Stage~1 & Encoder family \\
        ...
        S2-E1 & 2 & 84,380-flow manifest; same frozen embeddings & No-context MLP; Transformer; LSTM; GRU; CNN+LSTM; proposed & Context encoder \\
...
Figure~\ref{fig:results_stage1_sensitivity} reports sequence-length and packet-selection sensitivity.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{\resultsfigurepath/stage1_sensitivity.pdf}
    \caption{Stage~1 sensitivity analysis. Left: maximum packet-sequence length under head selection. Right: packet-selection strategy at $L=128$. Dashed lines identify the principal sequence length. Truncated axes are explicitly labelled.}
    \label{fig:results_stage1_sensitivity}
\end{figure}

Performance is non-monotonic in sequence length. Very short inputs have weaker ranking performance, while $L=128$ gives the best macro-F1 and error balance. Increasing the budget to 256 adds 46 FP and lowers both macro-F1 and PR-AUC, showing that a larger packet budget does not necessarily add useful evidence. Because the sweep changes truncation rates differently for short and long flows, $L=128$ is the best observed budget for this capture rather than a universal packet horizon.

Head and head-tail selection differ by only 0.16 percentage points of macro-F1. Random selection lowers FPR but increases FN, indicating a more conservative operating point. The principal head policy is deterministic and preserves the earliest packet prefix, while also providing the strongest observed macro-F1 and positive coverage. The small head versus head-tail difference is treated as descriptive.
...
Source-host context gives the strongest balance, with macro-F1 $0.9275$, PR-AUC $0.9345$, and the lowest FPR. Source-host histories group repeated outbound connections from the same initiator, whereas global recency mixes unrelated flows and destination histories can combine many benign clients of a shared service. This structural distinction is consistent with the observed source-host advantage and with relational and window-level NIDS findings \cite{Deng2023FlowTopology,MartinReizabal2026FlowWindow}.

Endpoint context occupies a high-recall regime: it misses fewer positives but produces 702 FP. Accepting either endpoint in either role produces a broader and more heterogeneous neighborhood, consistent with its higher recall and false-alert count. It may therefore be preferable when missed attacks dominate alert-review cost, whereas source-host is the better default under the thesis's balanced criterion. Time-only context performs worse in ranking and FN, indicating that proximity without an entity relation is too weak a context definition for this capture.
...
\begin{figure}[H]
    ...
    \caption{Source-host context-window sensitivity. Left: macro-F1 and PR-AUC. Right: false-positive and false-negative counts. The dashed line marks the principal window $W=128$.}
    \label{fig:results_stage2_window}
\end{figure}

As shown in Figure~\ref{fig:results_stage2_window}, the response to context length is non-monotonic. A 32-flow window produces the highest macro-recall, but its 775 false positives indicate that the added sensitivity is operationally costly. The 128-flow window instead gives the best joint result in macro-F1 ($0.9275$), PR-AUC ($0.9345$), and FPR ($0.5429\%$). Extending the window to 256 degrades both false-positive and false-negative counts. This pattern is consistent with a finite useful context horizon: too little history can omit a behavioural pattern, whereas older same-source flows may be unrelated to the current event and dilute the attention signal.

The non-monotonic response indicates that context quality matters more than context size alone. Because the archived summaries do not retain the occupancy and age distribution of valid context items, the experiment cannot separate the effects of older evidence, padding, and unequal history coverage. Accordingly, $W=128$ is the best empirical operating point in this capture rather than a universal optimum.

\subsection{Stage~2 Architecture Comparison}
\label{subsubsec:results_stage2_architecture_comparison}

The architecture comparison holds the Stage~1 embeddings, source-host relation, $W=128$, online causal policy, training objective, optimizer, sampling rule, checkpoint criterion, and validation threshold search fixed. Only the context encoder and its architecture-specific dimensions change.
...
The target-query model provides the strongest overall balance: it reaches macro-F1 $0.9275$, class-1 F1 $0.8610$, and PR-AUC $0.9345$. The closest baseline values are $0.9097$ macro-F1 from CNN+LSTM and $0.9141$ PR-AUC from GRU. LSTM attains marginally higher macro-recall, but this isolated advantage does not translate into the best threshold-independent ranking or the best false-alert balance.

The confusion matrices clarify the practical meaning of this ranking. Relative to the vanilla Transformer, the proposed model removes 177 false positives and 117 false negatives on the same test manifest. LSTM detects 28 additional positive flows, but produces 338 additional false alerts. Thus, the proposed model's macro-F1 improvement is not obtained by simply shifting the threshold towards conservative positive prediction; it reduces both error types against the vanilla attention baseline and substantially improves alert precision over the highest-recall recurrent baseline.

The comparison with vanilla self-attention is consistent with the intended inductive bias. Vanilla attention repeatedly updates the context sequence and classifies its final valid state. The proposed model instead asks a narrower question: which earlier related flows are relevant to the current flow? It uses the target as the query and gates the resulting context update. The simultaneous gain in macro-F1 and PR-AUC suggests that this target-centred computation ranks minority examples more effectively, although the comparison cannot isolate the query pattern from all architectural and parameter-count differences.

The no-context head contains 514 parameters, compared with 283,522 in the proposed Stage~2 model, and therefore serves as a current-flow-only lower bound rather than a capacity-matched ablation. Its result shows that a minimal independent classifier is insufficient, but the full performance difference cannot be attributed exclusively to historical context.

\subsection{Efficiency and Operational Trade-Offs}
\label{subsec:results_statistical_efficiency}

Figure~\ref{fig:results_stage2_efficiency} relates parameter count to measured model-evaluation latency and macro-F1.
...
Stage~1 supports modelling packets as an ordered, time-aware sequence rather than relying only on flow aggregates. Positional order and elapsed time provide complementary evidence, and flow-conditioned token modulation further improves the fused representation. A 128-packet head sequence gives the strongest observed balance, while the non-monotonic length response shows that increasing the token budget alone is insufficient.

\paragraph{RQ2: cross-flow context.}
Stage~2 supports contextual reasoning beyond independent flow classification. A 128-flow source-host history gives the strongest tested balance and reduces both false positives and false negatives relative to independent Scheme~C prediction. Endpoint context favours recall at a higher false-alert cost, confirming that relation choice affects the operating trade-off. The target-query model also outperforms the tested recurrent, convolutional--recurrent, vanilla-attention, and minimal no-context alternatives.

Together, the results answer the central research question: packet timing and order improve the individual-flow representation, while selected earlier flows provide complementary evidence for the final decision. The contribution is the evaluated hierarchy and its controlled decomposition, rather than the use of a Transformer in isolation.
